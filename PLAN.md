...
`--force-with-lease` 重推。同一趟带上 `--replace-text`：`2026-09-20-m10-columns-ram.jsonl` 里有
两行把测试库记成了带本机用户名的绝对 `%TEMP%` 路径，另外 12 个 `jsonl` 走的是 `$env:TEMP` 所以
本来就没这问题。代价是**本仓所有 SHA 都换了**，本文与 ADR 里引用的每一枚号（`09ef5aa`、
