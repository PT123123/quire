...
use quire_data::core::persistence::{Change, Repository, StorageError};
use quire_data::core::types::{
    Attachment, AttachmentId, Block, BlockId, BlockKind, Lang, OrderKey, Page, PageFont, PageId,
    PersistedState,
};
use quire_data::storage::backup;
use quire_data::storage::migrations;
use quire_data::storage::SqliteRepository;
use quire_data::testing::ScratchDir;
...
fn block(id: u64, page_id: u64, parent: Option<u64>, ord: u64, text: &str) -> Block {
    Block {
        ...
        checked: false,
        marks: Vec::new(),
        color: quire_data::core::ColorKind::Default,
        background: quire_data::core::ColorKind::Default,
        page_ref: None,
        folded: false,
        attachment: None,
        img_percent: 100,
...
fn a_grid_and_its_cells_round_trip_through_storage() {
    use quire_data::core::{Mark, MarkKind};
    let dir = tempfile();
...
/// `cargo test -p quire-data --test storage -- --ignored --nocapture save_latency`
#[test]
...
fn block_colors_and_page_moves_round_trip() {
    ...
    repo.apply(&[Change::BlockColorSet {
        id: BlockId(10),
        color: quire_data::core::ColorKind::Red,
        background: quire_data::core::ColorKind::Yellow,
    }])
    .unwrap();
    // ...and survive a full reload
    let state = repo.load().unwrap();
    let b = state.blocks.iter().find(|b| b.id == BlockId(10)).unwrap();
    assert_eq!(b.color, quire_data::core::ColorKind::Red);
    assert_eq!(b.background, quire_data::core::ColorKind::Yellow);
    let b = state.blocks.iter().find(|b| b.id == BlockId(11)).unwrap();
    assert_eq!(b.color, quire_data::core::ColorKind::Default);
    ...
    assert_eq!(moved.color, quire_data::core::ColorKind::Red);
...
mod database_layer {
    ...
    fn the_v16_step_adds_reference_indexes_to_a_v15_database() {
        use quire_data::core::{Mark, MarkKind};
        use quire_data::storage::backlinks;
...
mod database_property_layer {
    ...
    use quire_data::core::database_property::iso_date;
