...
use quire_data::core::types::{Block, BlockId, BlockKind, Lang, OrderKey, Page, PageFont, PageId};
use quire_data::services::search_service::SearchService;
use quire_data::services::settings_store::{Settings, SettingsStore};
use quire_data::storage::backup::{self, KEEP};
use quire_data::storage::{Database, SqliteRepository};
...
/// `cargo test -p quire-data --test backup -- --ignored --nocapture`.
#[test]
#[ignore = "one-off cost probe, not an assertion"]
fn snapshot_cost() {
    use quire_data::core::types::PersistedState;
...
mod version_files {
    ...
    use quire_data::storage::versions as v;
