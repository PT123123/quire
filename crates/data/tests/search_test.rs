...
use quire_data::core::persistence::{Change, Repository};
...
use quire_data::storage::search_index::SearchRequest;
...
fn a_v1_database_is_backfilled_on_upgrade() {
    ...
    let repo = Arc::new(SqliteRepository::open(&path).unwrap());
    {
        let conn = rusqlite::Connection::open(&path).unwrap();
        assert_eq!(
            quire_data::storage::migrations::user_version(&conn).unwrap(),
            quire_data::storage::migrations::CURRENT_VERSION
        );
    }
