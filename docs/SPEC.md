...
—— 2026-09-24 已交付。`ui/components/OrganizerArea.slint` 是唯一的新组件。笔记行给标题 / 摘要 / 置顶 / 标签 / 相对时间；任务行给勾选框 / 优先级条 / 标题 / 子任务计数 / 标签 / 清单色点 / 截止徽章；详情面板是那一行的全部字段（清单、优先级、截止、重复、标签、子任务、备注），子任务是**点一下才变成输入框**（不同行不能共用一个 draft，这是本仓"同一时刻只有一个活动输入"的老规矩）。行高等几何是桌面的（28 / 30 / 46 px），触屏镜像用 `touch-mode` 换成 44 px，快捷键提示在触屏一律不画。

场景：`notes` / `notes-detail` / `notes-search` / `tasks` / `tasks-detail` / `tasks-list` / `tasks-overdue` 及四个 `dark-` 臂。
