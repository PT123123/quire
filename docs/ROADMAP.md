...
| M8 Windows RC | Crash recovery, migrations, packaging/installer, Markdown import/export, settings UI, keyboard shortcut pass | 🔄 in progress 2026-09-20 · delivered: rotating snapshots + startup recovery (ADR-0015), the `session.meta` + rotating log + panic hook and the clean-exit marker (ADR-0018), data-location move to the per-user library (ADR-0020), schema migrations to v5, Markdown import/export, installer + `just dist`, LAN share/pull, settings UI incl. the STORAGE row (data folder / open-folder / back-up-now), Page and Link-to-page blocks, rich clipboard paste, Move page, the shortcut pass. The tail A-package is landing: A1 `--portable` ✅, A3 release-profile audit ✅ (pinned as ADR-0024), A5 this refresh, A2 first-paint measurement ✅ (the old window-up number understated startup ≈4×; see `docs/PERFORMANCE.md`), A6 installer end-to-end ✅ (`install/verify-installer.ps1`), A4 visual sweep ✅ (34 scenes light + dark at `42cb158`, 13 pass / 21 fail, report in `.scratch/sweep/report.md`, harness `benchmarks/scripts/sweep.ps1`). Still open: the user IME acceptance pass (M4); the A4 defect list — 3 HIGH (inline-mark runs overprint their base text and do not wrap; Settings' SHORTCUT labels escape the dialog card and print over the document; a tall Move-to popup is clipped by the window bottom — the list already scrolls, only the popup's max height is unclamped); menu-overflow polish (same root cause as the third HIGH); Explorer drag-and-drop import is deferred by a platform limit (Slint 1.18 delivers no external file-drop events — revisit on upgrade or via a Win32 `IDropTarget` in `platform/`) |
